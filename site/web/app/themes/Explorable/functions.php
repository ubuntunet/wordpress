...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		require_once( $template_directory . '/includes/post_thumbnails_explorable.php' );
...
function et_explorable_load_scripts_styles(){
	...
	if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'Explorable' ) ) {
		...
		wp_enqueue_style( 'explorable-fonts-open-sans', esc_url( add_query_arg( $query_args, "$protocol://fonts.googleapis.com/css" ) ), array(), null );
	}
	...
	if ( 'off' !== _x( 'on', 'Lobster font: on or off', 'Explorable' ) ) {
		...
		wp_enqueue_style( 'explorable-fonts-lobster', esc_url( add_query_arg( $query_args, "$protocol://fonts.googleapis.com/css" ) ), array(), null );
	}

	wp_enqueue_script( 'explorable-google-maps-api', esc_url( add_query_arg( array( 'key' => et_pb_get_google_api_key(), 'callback' => 'initMap' ), is_ssl() ? 'https://maps.googleapis.com/maps/api/js' : 'http://maps.googleapis.com/maps/api/js' ) ), array( 'jquery' ), et_get_theme_version(), false );
...
?>
	<p>
		...
		<input class="upload_image_button" type="button" value="<?php esc_html_e( 'Upload Image', 'Explorable' ); ?>"  data-choose="<?php esc_attr_e( 'Choose Header Background Image', 'Explorable' ); ?>" data-update="<?php esc_attr_e( 'Use For Header Background', 'Explorable' ); ?>" /><br/>
...
function et_explorable_admin_scripts_styles( $hook ) {
	...
	if ( 'listing' == $typenow ){
		...
		wp_enqueue_script( 'google-maps-api-admin', esc_url( add_query_arg( 'key', et_pb_get_google_api_key(), is_ssl() ? 'https://maps.googleapis.com/maps/api/js' : 'http://maps.googleapis.com/maps/api/js' ) ), array( 'jquery' ), '1.0', true );
...
function et_is_mobile_browser() {
	$et_is_mobile = (bool) preg_match( '#\b(ip(hone|od)|android\b.+\bmobile|opera m(ob|in)i|windows (phone|ce)|blackberry' . '|s(ymbian|eries60|amsung)|p(alm|rofile/midp|laystation portable)|nokia|fennec|htc[\-_]' . '|up\.browser|[1-4][0-9]{2}x[1-4][0-9]{2})\b#i', $_SERVER['HTTP_USER_AGENT'] );
